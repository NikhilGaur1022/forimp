...
import CollaboratorPicker from '../components/articles/CollaboratorPicker';
...
const SubmissionPage = ({ isEdit = false, articleId = null }) => {
  ...
  const handleCitationPrePopulation = (urlParams: URLSearchParams) => {
    const citeTitle = urlParams.get('cite_title') || '';
    const citeAuthor = urlParams.get('cite_author') || '';
    const citeDoi = urlParams.get('cite_doi') || '';
    const citeYear = urlParams.get('cite_year') || new Date().getFullYear().toString();
  ...
  const addReference = (): void => {
    if (newReference.title.trim() && typeof newReference.authors === 'string' && newReference.authors.trim()) {
      const reference: Reference = {
        ...newReference,
        id: Date.now(),
        authors: (newReference.authors as string).split(',').map(a => a.trim())
      };
      setForm(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Extract collaborators and exclude from article data
      const { collaborators, ...articleFields } = form;
      ...
      // Save collaborators (clear old, insert new)
      if (newArticleId) {
        // Remove all previous collaborators for this article
        await supabase
          .from('article_collaborators')
          .delete()
        ...
        if (form.collaborators.length > 0) {
          const rows = form.collaborators.map((c: Collaborator) => ({
            article_id: newArticleId,
            user_id: c.id
          }));
          await supabase.from('article_collaborators').insert(rows);
        }
      }
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          {/* Citation Notice */}
          {citationNotice && (
            <div className="bg-dental-50 border border-dental-200 text-dental-800 px-4 py-3 rounded-lg mb-6 flex items-start gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={addKeyword}
                  className="px-4 py-2 bg-dental-600 text-white rounded-lg hover:bg-dental-700 transition-colors"
                >
              ...
              {form.keywords.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {form.keywords.map((keyword, index) => (
                    <span
                      key={index}
                      className="inline-flex items-center gap-1 px-3 py-1 bg-dental-100 text-dental-800 rounded-full text-sm"
                    >
                      {keyword}
                      <button
                        type="button"
                        onClick={() => removeKeyword(index)}
                        className="hover:bg-dental-200 rounded-full p-0.5"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </span>
            ...
            <div className="space-y-4">
              ...
              <CollaboratorPicker
                ...
                onChange={collabs => setForm(prev => ({ ...prev, collaborators: collabs }))}
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 border border-gray-200 rounded-lg bg-gray-50">
                ...
                <button
                  ...
                  className="mt-4 flex items-center gap-2 px-4 py-2 bg-dental-600 text-white rounded hover:bg-dental-700 transition-colors"
                >
                  <Plus className="w-4 h-4" />
                  Add Reference
                </button>
            ...
            <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-dental-600 text-white rounded-lg hover:bg-dental-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {isSubmitting ? (
                  <>
