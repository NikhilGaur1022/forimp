...
const DentistDetailPage = () => {
  ...
  const [dentistStats, setDentistStats] = useState<DentistStats | null>(null);
  const [achievements, setAchievements] = useState<Achievement[]>([]);
  const [courses, setCourses] = useState<Course[]>([]);
  const [featuredDentists, setFeaturedDentists] = useState<FeaturedDentist[]>([]);
  ...
  const currentDentistIdRef = useRef<string | null>(null);

  useEffect(() => {
    if (id) {
      // Only reset the flag if we're loading a different dentist
      if (currentDentistIdRef.current !== id) {
        viewsIncrementedRef.current = false;
        currentDentistIdRef.current = id;
      }
  ...
  const fetchDentistData = async () => {
    ...
    try {
      ...
      // Step 1: Fetch dentist profile with related data using nested query
      ...
      if (dentistError) {
        console.error('Dentist query error:', dentistError);
        throw dentistError;
      }
      ...
      // Set the main dentist data
      setDentist(dentistData as Dentist);

      // Extract related data
      setDentistStats((dentistData.professor_stats as DentistStats[])?.[0] || null);
      setAchievements((dentistData.professor_achievements as Achievement[]) || []);
      setCourses((dentistData.courses as Course[]) || []);

      // Step 2: Fetch dentist's articles separately
      ...
      if (articlesError) {
        ...
        console.warn('Could not fetch articles for dentist');
      }

      setRecentArticles((articlesData as Article[]) || []);

      // Step 3: Fetch featured dentists
      ...
      if (!featuredError) {
        setFeaturedDentists((featuredData as FeaturedDentist[]) || []);
      }

      console.log('Dentist data loaded successfully');
        ...
        <PageContainer>
          <div className="flex items-center justify-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="text-gray-600 text-lg">Loading dentist profile...</p>
            </div>
          </div>
        </PageContainer>
      </div>
      ...
      <div className="pt-20 pb-16 min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 font-inter">
        <PageContainer>
          <div className="text-center py-12">
            ...
            <Link
              to="/professors"
              className="inline-flex items-center px-6 py-3 bg-dental-600 text-white rounded-lg hover:bg-dental-700 transition-colors"
            >
          ...
          <main className="col-span-12 lg:col-span-8">
              ...
              <div className="px-8">
                <div className="flex flex-col lg:flex-row items-center gap-8">
                  ...
                  <div className="flex-1 text-center lg:text-left">
                    <h1 className="text-4xl font-bold mb-4">{dentist.full_name}</h1>
                    
                    <div className="space-y-2 mb-6">
                      <div className="flex items-center justify-center lg:justify-start text-white/90">
                        <GraduationCap className="w-5 h-5 mr-2" />
                        <span className="text-lg">{dentist.position}</span>
                      </div>
                      
                      <div className="flex items-center justify-center lg:justify-start text-white/90">
                        <MapPin className="w-5 h-5 mr-2" />
                        <span>{dentist.institution || dentist.university}</span>
                      </div>

                      {dentist.professor_since && (
                        <div className="flex items-center justify-center lg:justify-start text-white/90">
                          <Calendar className="w-5 h-5 mr-2" />
                          <span>Dentist since {new Date(dentist.professor_since).getFullYear()}</span>
                        </div>
                      )}
                    </div>
            ...
            {dentistStats && (
              <section className="mb-12">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Professional Performance</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div className="text-center bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl">
                    <BookOpen className="w-8 h-8 text-blue-600 mx-auto mb-2" />
                    <div className="text-2xl font-bold text-gray-900">{dentistStats.articles_published || 0}</div>
                  ...
                  <div className="text-center bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{dentistStats.total_citations || 0}</div>
                  ...
                  <div className="text-center bg-gradient-to-br from-purple-50 to-purple-100 p-6 rounded-xl">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{dentistStats.courses_count || 0}</div>
                    <div className="text-sm text-gray-600">Courses</div>
                  </div>
                  
                  <div className="text-center bg-gradient-to-br from-orange-50 to-orange-100 p-6 rounded-xl">
                    <Users className="w-8 h-8 text-orange-600 mx-auto mb-2" />
                    <div className="text-2xl font-bold text-gray-900">{dentistStats.students_taught || 0}</div>
