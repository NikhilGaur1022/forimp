// src/components/professors/DentistCard.tsx
...
type DentistCardProps = {
  dentist: ProfessorCardData;
  className?: string;
};

const DentistCard: React.FC<DentistCardProps> = ({ dentist, className = '' }) => {
  ...
  return (
    <div className={`group relative ${className}`}>
      <Link 
        ...
        className="block"
      >
        <div className={`relative bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 cursor-pointer ${dentist.is_featured ? 'h-[450px]' : 'h-[420px]'}`}>
          ...
          <div className={`px-5 pb-3 ${dentist.is_featured ? 'pt-12' : 'pt-6'}`}>
          ...
          <div className="px-5 pb-3 h-40">
            <div className="relative h-full">
              <div className={`w-full h-36 rounded-2xl ${getSpecialtyColor(dentist.specialty)} p-1 shadow-lg`}>
          ...
          <div className="px-5 pt-4 pb-4 h-32 flex flex-col">
            ...
            <div className="h-12 mb-2">
              ...
              <div className="flex items-center text-gray-600">
                ...
                <span className="text-xs truncate">{dentist.institution}</span>
              </div>
            </div>
            ...
            <div className="h-6 flex items-center justify-between mb-3">
              <div className="flex items-center bg-neutral-50 px-2 py-1 rounded-md">
                <BookOpen className="w-3 h-3 mr-1 text-neutral-600" />
                <span className="text-neutral-700 font-medium text-xs">{dentist.publications_count}</span>
              </div>
              <div className="flex items-center bg-neutral-50 px-2 py-1 rounded-md">
                <GraduationCap className="w-3 h-3 mr-1 text-neutral-600" />
                <span className="text-neutral-700 font-medium text-xs">{dentist.courses_count}</span>
              </div>
            </div>
